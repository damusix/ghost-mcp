...
import { getAction, type ApiType } from "../actions/registry.js";
...
function extractQueryParams(payload: Record<string, unknown>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(payload)) {
    if (PATH_PARAMS.includes(key as (typeof PATH_PARAMS)[number])) continue;
...
export async function handleUseGhostApi(input: UseGhostApiInput, mode: string): Promise<string> {
  ...
  if (mode === "content" && api === "admin") {
    return JSON.stringify({
      error:
        "Admin API is not available in content mode. Set GHOST_API_MODE=admin and provide GHOST_ADMIN_API_KEY to use admin actions.",
    });
  }
...
async function handleFileUpload(
  ...
  if (fileInput.startsWith("http://") || fileInput.startsWith("https://")) {
    ...
    filename =
      urlPath.split("/").pop() || (actionName === "images.upload" ? "image.jpg" : "theme.zip");
  } else {
    // Base64
