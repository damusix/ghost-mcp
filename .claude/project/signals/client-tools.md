...
- [`src/koenig/util.ts`](../../../src/koenig/util.ts) — `isRecord(value)`: type guard returning `value is Record<string, unknown>`; used by `blocks.ts` and `compose.ts` to narrow `unknown` inputs before field access
...
- [`src/tools/blocks-source.ts`](../../../src/tools/blocks-source.ts) imports `isRecord` from [`src/koenig/util.ts`](../../../src/koenig/util.ts) — `util.ts` is a shared utility; changes to `isRecord` affect both `blocks-source.ts` and `compose.ts`
...
- 103 tests pass; new test coverage added in [`src/__tests__/tools/blocks-source.test.ts`](../../../src/__tests__/tools/blocks-source.test.ts) covering inline array, `blockFile` absolute path, conflict error, missing-both error, non-absolute path error, read failure, bad JSON, and the `{ "blocks": [...] }` wrapper shape
