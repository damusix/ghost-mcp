...
- [`src/actions/registry.ts`](../../../src/actions/registry.ts) — central registry: `ActionDefinition` interface, `HttpMethod`/`ApiType` types, Map store, all register/query/help exports, `describeZodType`/`unwrapZodType` helpers for schema introspection, `initRegistry` that imports and bulk-registers all action arrays
...
- [`src/actions/admin/offers.ts`](../../../src/actions/admin/offers.ts) — 4 admin offer actions: browse, read, add, edit (no delete)
...
- [`src/actions/admin/webhooks.ts`](../../../src/actions/admin/webhooks.ts) — 3 admin webhook actions: add, edit, delete (no browse/read)
...
- [`src/actions/registry.ts`](../../../src/actions/registry.ts) exports `ActionDefinition`, `HttpMethod`, `ApiType`, `getAction`, `listActions`, `getActionHelp` — any MCP tool handler that resolves or describes Ghost API calls imports from this module; adding or renaming an action here requires updating any consumer that references actions by name
- All action files import `ActionDefinition` as a type from `../registry.js`; changing the `ActionDefinition` interface shape forces updates across all 18 action files
- [`src/__tests__/registry.test.ts`](../../../src/__tests__/registry.test.ts) and `src/__tests__/actions/*.test.ts` call `getAction`/`listActions`/`getActionHelp` directly — test assertions are bound to registered action names, methods, and paths
...
- `adminOfferActions` defines `offers.browse` with an empty Zod object schema (`z.object({}).describe(...)`) — the browse endpoint takes no query parameters
