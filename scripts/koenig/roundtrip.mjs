...
async function roundtrip(name, node) {
    const body = JSON.stringify({
        posts: [{ title: `[probe] ${name}`, status: 'draft', lexical: doc(node) }],
    });
    const post = await fetch(`${URL_}/ghost/api/admin/posts/?formats=html,lexical`, {
        method: 'POST',
        headers: hdr(),
        body,
    });
    ...
    if (post.status !== 201) {
        return {
            ...
            ok: false,
            status: post.status,
            error: JSON.stringify(j.errors?.[0]?.context || j.errors?.[0]?.message || j).slice(
                0,
                200,
            ),
        };
    ...
    return {
        ...
        storedKeys: stored ? Object.keys(stored).join(',') : '(dropped)',
    };
}
...
for (const [name, node] of Object.entries(cards)) {
    try {
        results.push(await roundtrip(name, node));
    } catch (error) {
...
for (const r of results) {
    if (r.id) {
        await fetch(`${URL_}/ghost/api/admin/posts/${r.id}/`, { method: 'DELETE', headers: hdr() });
    }
}
...
for (const r of results) {
    if (r.ok) {
        ...
        console.log(
            r.name.padEnd(18),
            '201',
            renders.padEnd(5).slice(0, 5),
            (r.storedType || '—').padEnd(16),
            r.htmlSnippet || '',
        );
    } else {
        console.log(
