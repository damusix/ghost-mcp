...
function token() {
  ...
  const h = b64({ alg: 'HS256', typ: 'JWT', kid: id }), p = b64({ iat: now, exp: now + 300, aud: '/admin/' });
...
const lexical = JSON.stringify({ root: { type: 'root', version: 1, direction: 'ltr', format: '', indent: 0, children } });

// delete any prior gallery, then create fresh
const existing = await (await fetch(`${URL_}/ghost/api/admin/posts/slug/koenig-card-gallery/`, { headers: hdr() })).json();
if (existing.posts?.[0]) await fetch(`${URL_}/ghost/api/admin/posts/${existing.posts[0].id}/`, { method: 'DELETE', headers: hdr() });

const body = JSON.stringify({ posts: [{ title: 'Koenig Card Gallery', slug: 'koenig-card-gallery', status: 'published', lexical }] });
