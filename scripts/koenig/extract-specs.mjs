...
const ROOT = new URL('../../tmp/koenig-repo/packages/kg-default-nodes/src/nodes', import.meta.url)
    .pathname;
...
for (const file of walk(ROOT)) {
    ...
    for (const line of src.split('\n')) {
        const fm = line.match(
            /\{name:\s*'([^']+)',\s*default:\s*(.+?)(?:,\s*(?:wordCount|urlType|urlPath|urlTransformMap):.*)?\}[,\s]*$/,
        );
        if (fm) {
            let def = fm[2]
                .trim()
...
const elementNodes = {
    'extended-quote': {
        kind: 'element',
        note: 'blockquote; holds inline children (extended-text/link)',
    },
...
for (const [t, v] of Object.entries(elementNodes)) {
    specs[t] = { nodeType: t, hasVisibility: false, fields: {}, ...v };
}

writeFileSync(
    new URL('../../docs/koenig-node-specs.json', import.meta.url),
    JSON.stringify(specs, null, 2),
);
const decorator = Object.values(specs).filter((s) => Object.keys(s.fields).length).length;
console.log(
    `wrote docs/koenig-node-specs.json — ${Object.keys(specs).length} node types (${decorator} with decorator fields)`,
);
