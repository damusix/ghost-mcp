...
for (const [name, node] of Object.entries(cards)) {
  const body = JSON.stringify({ posts: [{ title: `[cap] ${name}`, status: 'draft', lexical: doc(node) }] });
  const r = await fetch(`${URL_}/ghost/api/admin/posts/?formats=lexical`, { method: 'POST', headers: hdr(), body });
