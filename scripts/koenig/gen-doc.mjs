...
const n = JSON.parse(
    readFileSync(new URL('../../docs/koenig-cards.json', import.meta.url), 'utf8'),
);
const specs = JSON.parse(
    readFileSync(new URL('../../docs/koenig-node-specs.json', import.meta.url), 'utf8'),
);
...
for (const g of groups) {
    ...
    for (const k of keys) {
        ...
        md += `\n\n### ${k}\n\n${note}\n\n_Renders: ${render}._`;
        if (sp?.hasVisibility) {
            md += ` Supports a \`visibility\` object (web/email member segments).`;
        }
